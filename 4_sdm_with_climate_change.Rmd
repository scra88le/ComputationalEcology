...
# Future climate data
bio_fut <- getData('CMIP5', var='bio', download=T, res=5, rcp=45, model='MC', year=50, path=filepath)
...
#library(sf)
# Create a sf set of coordinates
#df_sp <- st_as_sf(zonaria_clean[1:2], 
                  #coords = c("decimalLongitude","decimalLatitude"), 
                 # crs=4326)

# Create coordinates
#df_sp$x <- st_coordinates(df_sp)[,1]
#df_sp$y <- st_coordinates(df_sp)[,2]
...
Now we can now make a prediction for both the current species distribution range for V. zonaria. and a future 2050 scenario based on the CMIP5 climate change model.
...
# Generate sdm under climate change scenario
fut_plot  <- predict_dist(bio_fut, model_fit) %>%
...
We see a sigificant incerase in range in the north and west of the UK.
