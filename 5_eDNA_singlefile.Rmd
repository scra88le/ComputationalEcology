...
For this particular study, demultiplexed reads from from a MiSeq DNA sequencing run were generated from a scientific study of seston DNA (environmental DNA or “eDNA” represented in suspended particulate matter) sampled from an urban watershed of size 42,9994ha in southwestern Ohio, USA. [^1]. Illumina MiSeq fastq files were downloaded from the NCBI Sequence Read Archive (SRA). The The SRA stores raw sequencing data and alignment information from high-throughput sequencing platforms. 

We will aim to use a machine learning classifier to generate Operational Taxonomic Units (OTUs) from the amplicons within the raw fastq files. In this particular example,  OTUs are clusters of similar sequence variants of the 18S rDNA marker gene sequence. Each of these clusters is intended to represent a taxonomic unit of a eukaryote, that has been resolved to a species or genus. In order to achieve this we use a reference taxonomy database of Eukaryotic 18S rDNA.
...
taxa_df[1:7] %>% 
  ...
  ggplot(aes(Class)) +
    ...
    coord_flip() +
    theme_bw() +
    ggtitle("Distribution of class taxa")

```

# Discussion

We have seen that it is possible to take raw amplicon data that has been sequenced using environmental DNA, and identify which species are present in the underlying field samples. Environmental DNA sequencing and  bioinformatic techniques are a highly productive species surveying tool, especially in environments where the taget taxa are not easily identifiable in the field or considerable taxanomic expertise is required by the scientist undertaking the survey.
